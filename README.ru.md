...
решила и о чём вспомнила. Покрытие — 85 unit-тестов, корпус из 79 кейсов
на обходы и ложные срабатывания и фаззинг разбора команд.
...
| `secret-scan` | секреты в открытом виде (`PostToolUse`) |
...
3. **Замки** — 85 кейсов: JSON на вход, решение `allow/deny/ask` на выход.
   Отдельно проверяется поведение без `jq`: замок обязан отказать, а не
   промолчать.
4. **Обходы и ложные срабатывания** — корпус из 79 кейсов
   ...
   целиком.
5. **Автодетект и связи** — 33 кейса, включая регрессии на найденные баги.
6. **Профили, храповик и настройка** — 63 кейса: определение профиля
...
7. **Политика стека** — 20 кейсов, в том числе: проект вне политики сохраняет
   замки безопасности, а проект без файла политики не трогается вовсе.
8. **Правила уровня проекта** — 26 кейсов: шаблон, файл приоритета, запись
   отступления и разделение на коммитимые правила проекта и симлинки общих.
9. **Публикация** — 22 кейса, в том числе: релиз не создаётся, пока прогон
   на GitHub не зелёный.
10. **`claude plugin validate --strict`** по каждому модулю.
