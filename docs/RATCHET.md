...
(Infection) или `--incremental` (Stryker: пересчитываются только мутанты
в изменённом коде, остальные берутся из прошлого прогона). Флага `--since`,
который здесь стоял раньше, у Stryker нет — прогон падал с «unknown option».
...
Осмысленно для статических сайтов и прототипов: мутировать нечего. Гейты
мутации кода при этом пропускаются, даже если остались в `gates` (например,
после смены профиля на `prototype`), — в итоге прогона они видны как
«выключен». Мутация данных спецификации (`spec-mutation`) — отдельная
проверка, этот ключ её не выключает.
...
Что делать: разобрать выживших мутантов и усилить ассерты (скилл
mutation-harden). Если планку и правда нужно понизить (удалён хорошо
покрытый модуль), это решение человека: остановись и спроси человека.
```

Команды сброса в отказе нет намеренно. Сообщение читает агент, и готовая
команда читалась как следующий шаг: планку опускали, не спросив.
...
решением человека**, а не побочным эффектом автоматики. История при сбросе
не стирается — в ней появляется запись «установлено вручную», и видно, что
было до решения.

Значение проверяется до записи: целое от 0 до 100. `reset 70%` или `reset 7O`
отвергаются с кодом 2, прежняя планка остаётся на месте.

### Если состояние повреждено

Пустой или битый `.claude/.ratchet.json` — отказ с кодом 2, а не «планки
нет». Раньше такой файл читался как планка 0: проходил любой результат,
и это не лечилось само. Чинит человек тем же `ratchet.sh reset <значение>`.

Стартовая планка в настройках (`floor`) может быть дробной — берётся её целая
часть. Результат прогона принимается с точкой или запятой (`63,45` → 63).
...
Так умеют `phpstan --error-format=raw`, `ruff --output-format=concise`,
`mypy`. Для ESLint по умолчанию берётся `-f json`, а строки собирает `jq`:
форматтер `unix` из ESLint 9 убран. Строки-сводки инструмента в долг не
попадают — иначе число менялось бы при обновлении версии, а не при изменении
кода. Путь может быть абсолютным и содержать пробелы.

Код возврата команды читается. `0` и `1` — подсчёт состоялся (у линтеров
единица значит «нарушения найдены»). Всё, что выше, — инструмент упал или
не установлен: гейт отвечает «не измерено» с кодом 2 и планку **не трогает**.
Раньше упавший инструмент давал «ДОЛГ: 0», первый прогон фиксировал планку
на нуле, а после починки реальный долг навсегда оказывался ростом.

Повреждённое состояние `.claude/.debt.json` — тоже код 2, а не «планки нет».
`debt.sh reset` принимает только целое неотрицательное число.
...
линтер. Отказ гейта говорит агенту остановиться и спросить человека, а не
предлагает команду сброса.
