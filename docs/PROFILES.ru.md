...
|---|---|---|
| `plugins/std-core/profiles/profiles.json` | шесть готовых профилей | тот, кто ведёт репозиторий стандартов |
...
Без аргумента профиль определится сам — по числу авторов, количеству коммитов
и наличию тестов.
...
Нужно, когда у команды есть режим, не покрытый шестью готовыми. Например,
...
| `mutation` | `ratchet` от планки, `absolute` с порогом, или выключено |
...
Профили `corporate` и `regulated` намеренно **не** определяются автоматически:
понять по коду, что проект работает с деньгами или медданными, надёжно нельзя,
а ошибка дорогая.
...
- [Основы простыми словами](START.ru.md) — что такое гейт, храповик, замок
- [Как написать своё правило](WRITING-RULES.ru.md) — правила и профили не связаны
- [Строй проверок](../plugins/std-gauntlet/docs/GAUNTLET.md) — пять ступеней подробно
