...
```bash
/std-core:setup --no-hooks   правила подключены, проверки молчат
/std-core:setup --hooks      вернуть проверки
```

Создаётся `.claude/std-hooks-off`, и каждый хук стандартов, увидев этот файл,
выходит не проверяя. Правила при этом остаются: стандарт возвращается к тексту,
которому следуют по памяти. Гейты не затрагиваются — `/std-gauntlet:run`
запускается руками и от хуков не зависит.

Режим существует потому, что альтернатива ему хуже. Человек, которому вопросы
мешают работать, выключает защиту в любом случае; вопрос лишь в том, чем именно.
Штатный `disableAllHooks` гасит заодно чужие хуки, переменная окружения
не видна никому, кроме того, кто её поставил, а удалённый плагин уносит
с собой и правила. Здесь выключение — одно, видимое и обратимое: файл в дереве
проекта, о котором сессия напоминает на старте.

Три свойства, которые делают его решением, а не дырой:
...
- **выключение видно.** Файл лежит в проекте и попадает в `git status`;
  `session-check` называет цену в начале каждой сессии, чтобы «на время»
  не стало «навсегда»;
...
/std-core:setup --no-hooks  оставить одни правила, замки выключить
