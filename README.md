...
решила и о чём вспомнила. Покрытие — 137 unit-тестов, 41 тест на утечку
секретов, корпус из 97 кейсов на обходы и ложные срабатывания и фаззинг
разбора команд.

| Замок | Что не пропускает |
|---|---|
| `guard-bash` | удаление контейнеров, образов, томов и кэша; force-push, включая `+refspec` и `--mirror`; `--no-verify` и `commit -n`; подмену `core.hooksPath`; `migrate:fresh`, `DROP`/`TRUNCATE`, `dropdb` — в том числе в тексте на входе клиента БД; рекурсивное `rm` по системным путям **и их подпутям** (`/var/lib/postgresql`, `/etc/nginx`, чужой `/home/*`) после раскрытия `..`, `~`, `$HOME` и `{a,b}`; `find … -delete` по ним же; `dd`, `shred`, `wipefs`, `blkdiscard` на устройство; `mkfs`; `chmod`/`chown` корня и системных каталогов. Спрашивает перед потерей работы: `reset --hard`, `clean -f`, `branch -D`, `stash drop`, удаление ветки на сервере, `DELETE`/`UPDATE` без `WHERE`, перезапись системных файлов |
| `guard-tests` | **правку существующего теста** (новые пишутся свободно) — и через Write/Edit, и через `sed -i`, `>`, `cp`, `rm`, `git checkout -- tests/` |
| `guard-infra` | правку манифестов k8s, playbook'ов, конфигурации CI, Dockerfile, применённых миграций, `.env` — **и самого механизма проверок**: `.claude/settings.json`, правил проекта, конфигурации гейтов, git-хуков, исключений словаря секретов, состояния храповиков и отметки прохода гейтов |
| `guard-deps` | добавление зависимости прямой правкой манифеста: `composer.json`, `package.json`, `requirements*.txt`, `pyproject.toml`, `go.mod`, `Cargo.toml` — включая `latest`, `github:`, `git+`, `npm:`-псевдонимы, `postinstall` и чужие источники пакетов |
...
| `docker run -v /:/host … rm -rf /host/etc` | путь внутри контейнера не сопоставляется с примонтированным каталогом хоста |
| `cd "$DIR" && rm -rf *` | каталог из неизвестной переменной считается каталогом проекта |
| `psql -f drop.sql` | SQL из файла замок не читает, как и файл скрипта |
| `rm -rf $(cat dirs.txt)` | цель вычисляется при выполнении — в тексте команды её нет |
...
Команду больше полутораста подкоманд или с вложенностью глубже трёх обёрток
замок поштучно не разбирает: такая строка проверяется на запреты по тексту
целиком и уходит человеку с вопросом (при выключенных вопросах остаются
только запреты). Разбор любой длины укладывается в таймаут хука: команда,
которую хук не успел проверить, выполнилась бы без решения.
...
`bash` 4.4 или новее — тоже обязательно. В macOS по умолчанию стоит 3.2,
и на нём замки не падали, а работали с пустыми данными: `guard-bash` делил
команду на ноль частей и пропускал всё. Теперь замок сам ищет новый bash
(Homebrew, MacPorts, Nix) и перезапускается под ним; не нашёл — спрашивает
человека о каждом действии, а старт сессии называет причину и лекарство:
`brew install bash`.

### Правила без вопросов

```bash
/std-core:setup --no-hooks   вопросы выключены, запреты остаются
/std-core:setup --hooks      вернуть всё
```

Выключается не защита, а разговор. Мешают именно вопросы: они приходят на
команду, которую человек уже решил выполнить, и стоят внимания каждый раз.
Запреты не мешают никому — тот, кто не собирался удалять том, не заметит,
что удалять его нельзя.

| | В обычном режиме | С `--no-hooks` |
|---|---|---|
| `rm -rf` по системному пути или его подпути | запрет | **запрет** |
| `docker`/`podman`/`buildah` rm, rmi, prune, `down -v` | запрет | **запрет** |
| `drop`/`truncate`, `migrate:fresh`, `db:wipe` | запрет | **запрет** |
| `mkfs`, `dd of=/dev/sda`, `find / -delete` | запрет | **запрет** |
| force-push, `--no-verify`, подмена `hooksPath` | запрет | **запрет** |
| `rm -rf` вне проекта (`~/projects`, `/tmp/x`) | вопрос | **вопрос** |
| `reset --hard`, `clean -d`, `terraform destroy` | вопрос | **вопрос** |
| `rm -rf` внутри проекта (`node_modules`, `build/`) | свободно | свободно |
| новая зависимость, `cat .env`, `printenv` | вопрос | тихо |
| правка существующего теста | вопрос | тихо |
| секреты при записи и в коммите | проверка | тихо |
| образцы соседних файлов, инфраструктура | подсказка | тихо |

Запреты выживают и внутри обёрток: `bash -c "podman rmi old"` блокируется так же.
Правила, команды, скиллы и гейты работают как обычно — `/std-gauntlet:run`
запускается руками и от хуков не зависит.

Механизм — файл `.claude/std-hooks-off`: остальные хуки, увидев его, выходят
сразу, а `guard-bash` переключается на запреты без вопросов. Штатный
`disableAllHooks` так не умеет — он гасит всё и заодно чужие хуки.

Три свойства, которые делают это решением, а не дырой:

- **выключает только человек.** Пока замки живы, они охраняют собственный
  выключатель: создание маркера моделью эскалируется к человеку;
- **выключение видно.** Файл лежит в проекте и попадает в `git status`;
  `session-check` называет на старте каждой сессии, что именно перестало
  проверяться, — чтобы «на время» не стало «навсегда»;
- **возвращается одной командой** — и `--remove` убирает маркер вместе
  с остальной настройкой.

Если вопросы задаёт не замок, а сама Claude Code (штатное разрешение на Bash),
`--no-hooks` не поможет: это разные механизмы. Отличить просто — замок всегда
объясняет причину своими словами, штатный вопрос сух. Второй лечится списком
разрешений, а не отключением проверок.
...
/std-core:setup --no-hooks  выключить вопросы, оставить запреты на разрушающее
...
3. **Статический анализ** — `shellcheck -S warning` по всем shell-скриптам.
   В shell дефект часто выглядит как работающий код: неэкранированная
   переменная, маска вместо строки, `cd` без проверки. В CI версия
   закреплена, локально без shellcheck шаг называет себя пропущенным.
4. **Структура модулей** — манифесты, frontmatter, `owner`, `enforcement`,
   мёртвые `paths:`, размер постоянного контекста, доля неавтоматизированного.
5. **Замки** — 207 кейсов: JSON на вход, решение `allow/deny/ask` на выход.
   Отдельно проверяется поведение без `jq` и на bash старше 4.4: замок обязан
   спросить человека или перезапуститься под новым bash, а не промолчать.
   Настоящий bash 3.2 проверяется в CI на macOS (`tests/test-real-old-bash.sh`).
6. **Утечка секретов** — 91 кейс на всех четырёх точках: чтение, команда,
   запись, коммит — включая `git add . && git commit`, маску Grep поверх
   `.gitignore` и пути не в ASCII. Проверяется и обратное — что образцы, плейсхолдеры
   и значения из окружения проходят молча.
7. **Обходы и ложные срабатывания** — корпус из 302 кейсов
   ...
   целиком. Каждый запрет корпуса повторяется с выключенными вопросами:
   маркер `std-hooks-off` не должен снимать ни одного. В CI на macOS корпус
   идёт ещё и на системном BSD awk — разбор команд написан на awk.
8. **Автодетект и связи** — 33 кейса, включая регрессии на найденные баги.
9. **Профили, храповик и настройка** — 80 кейсов: определение профиля
...
10. **Храповик долга** — 29 кейсов: первый прогон фиксирует факт, рост валит
   гейт, снижение опускает планку навсегда, а «нечем измерить» и упавший
   инструмент не выглядят как «долга нет».
11. **Покрытие изменённых строк** — 35 кейсов на двух форматах отчёта: считаются
   только строки этой работы, новый файл вне индекса и файл, которого нет
   в отчёте, не дают ложные 100%, отсутствие отчёта и мелкий клон
   отличаются от успеха.
12. **Строй проверок и замок на коммит** — 55 кейсов: из отчёта мутаций
   берётся то самое число, нераспознанный отчёт — провал, а не пропуск,
   отметка прохода — отпечаток содержимого, и правка после прогона
   (в том числе `mv`, `rm`, любой тип файла) требует нового прогона.
13. **Политика стека** — 22 кейса, в том числе: проект вне политики сохраняет
   замки безопасности, а проект без файла политики не трогается вовсе.
14. **Правила уровня проекта** — 26 кейсов: шаблон, файл приоритета, запись
    отступления и разделение на коммитимые правила проекта и симлинки общих.
15. **Публикация** — 24 кейса, в том числе: релиз не создаётся, пока прогон
    на GitHub не зелёный.
16. **`claude plugin validate --strict`** по каждому модулю. В CI CLI ставится
    закреплённой версией — без него шаг пропускался, и зелёная галочка
    не включала эту проверку.
...
- внешний разбор `guard-bash.sh` и `gauntlet.sh` тем, кто их не писал —
  [разбор агентами проведён](docs/AUDIT.md), разбор человеком ещё нужен;
...
**[Внешний разбор](docs/AUDIT.md)** — что нашли в замках и гейтах те, кто
их не писал, что исправлено и что осталось.
