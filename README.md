...
|---|---|
| `guard-bash` | удаление контейнеров, образов, томов и кэша; force-push, включая `+refspec`; `--no-verify` и `commit -n`; подмену `core.hooksPath`; `migrate:fresh`, `DROP TABLE`/`DROP DATABASE`; рекурсивное `rm` по системным путям **и их подпутям** (`/var/lib/postgresql`, `/etc/nginx`, чужой `/home/*`); `find / -delete`; `dd` на устройство; `mkfs`; рекурсивный `chmod`/`chown` по системным путям |
...
5. **Замки** — 93 кейса: JSON на вход, решение `allow/deny/ask` на выход.
   Отдельно проверяется поведение без `jq`: замок обязан отказать, а не
   промолчать.
6. **Утечка секретов** — 41 кейс на всех четырёх точках: чтение, команда,
   запись, коммит. Проверяется и обратное — что образцы, плейсхолдеры
   и значения из окружения проходят молча.
7. **Обходы и ложные срабатывания** — корпус из 97 кейсов
...
9. **Профили, храповик и настройка** — 63 кейса: определение профиля
...
10. **Храповик долга** — 17 кейсов: первый прогон фиксирует факт, рост валит
   гейт, снижение опускает планку навсегда, а «нечем измерить» не выглядит
   как «долга нет».
11. **Покрытие изменённых строк** — 14 кейсов на двух форматах отчёта: считаются
   только строки этой работы, новый файл вне индекса не даёт ложные 100%,
   отсутствие отчёта отличается от успеха.
12. **Политика стека** — 20 кейсов, в том числе: проект вне политики сохраняет
   замки безопасности, а проект без файла политики не трогается вовсе.
13. **Правила уровня проекта** — 26 кейсов: шаблон, файл приоритета, запись
    отступления и разделение на коммитимые правила проекта и симлинки общих.
14. **Публикация** — 24 кейса, в том числе: релиз не создаётся, пока прогон
    на GitHub не зелёный.
15. **`claude plugin validate --strict`** по каждому модулю. В CI CLI ставится
