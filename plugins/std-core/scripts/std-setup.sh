...
# ── Замки ─────────────────────────────────────────────────────────────────────
# Выключается не защита, а разговор. Мешают вопросы: они приходят на команду,
# которую человек уже решил выполнить, и стоят внимания каждый раз. Запреты
# не мешают никому — тот, кто не собирался удалять том, не заметит, что удалять
# его нельзя. Поэтому маркер глушит вопросы и оставляет запреты, а не рубит
# всё сразу, как штатный disableAllHooks (тот гасит заодно чужие хуки).
#
# Остаётся: блокировка необратимого (rm -rf по системным путям, удаление
# образов и томов, drop, truncate, mkfs, dd, force-push), вопрос о потере
# сделанного (reset --hard, clean, terraform destroy), правила, команды,
# скиллы, гейты.
# Пропадает: вопросы про зависимости, секреты и неразобранные команды, замок
# на правку тестов, скан секретов при записи и в коммите, подсказки.
if [[ -n "$HOOKS" ]]; then
  echo; b "▸ Замки"
  if [[ "$HOOKS" == "0" ]]; then
    if [[ $DRY -eq 1 ]]; then
      ...
      mkdir -p "$CFG_DIR"
      cat > "$MARKER" <<EOF
# Вопросы замков в этом проекте выключены — $(date +%F).
#
# Что осталось. Запреты на необратимое работают по-прежнему: rm -rf по
# системному пути и из домашней, удаление образов, томов, сетей и кэша
# (docker, podman, buildah), compose down -v, drop и truncate, migrate:fresh,
# mkfs, dd на устройство, force-push. Плюс вопрос там, где теряется уже
# сделанное: git reset --hard, git clean, откат дерева, удаление namespace,
# terraform destroy. Этих команд за неделю единицы — они не мешают.
#
# Что выключено. Всё остальное: вопрос про новую зависимость, про чтение
# файла с доступами, про неразобранную команду; замок на правку существующих
# тестов; проверка секретов при записи и в коммите; подсказки по соседним
# файлам и по инфраструктуре.
#
# Правила (.claude/rules/std-*) действуют: модель их читает и им следует.
# Гейты тоже: /std-gauntlet:run запускается руками и от хуков не зависит.
#
# Вернуть всё: /std-core:setup --hooks (или просто удалить этот файл).
EOF
      grn "  создан .claude/std-hooks-off — вопросы выключены, запреты остались"
      echo "  Блокировка удаляющих команд работает: rm -rf по системным путям,"
      echo "  удаление образов и томов, drop и truncate, mkfs, force-push."
...
if [[ -f "$MARKER" && $DRY -eq 0 ]]; then
  ylw "  Вопросы замков выключены (.claude/std-hooks-off)."
  echo "  Запреты на удаляющее и необратимое работают. Не проверяются: правка"
  echo "  существующих тестов, секреты при записи и в коммите, новые зависимости."
  echo "  Профиль выше описывает планку качества — держать её теперь тебе."
  echo "  Вернуть всё: /std-core:setup --hooks"
  echo
fi
