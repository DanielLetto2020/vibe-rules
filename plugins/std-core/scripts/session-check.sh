...
if [[ -f "$PROJECT_DIR/.claude/std-hooks-off" ]]; then
  NOTES+=("Замки в этом проекте отключены (.claude/std-hooks-off). Ни одна проверка не выполняется: ни на разрушающие команды, ни на правку тестов, ни на секреты в файле и в коммите. Правила остаются текстом, и соблюдать их теперь некому, кроме тебя — держи их сам, особенно там, где потеря необратима. Вернуть проверки: /std-core:setup --hooks")
  HOOKS_WARN="Замки стандартов выключены (.claude/std-hooks-off): проверок на rm, force-push, миграции и секреты сейчас нет. Вернуть: /std-core:setup --hooks"
fi
