...
# ── Создание правила ──────────────────────────────────────────────────────────
# Шаблоны переписывания. Отличаются от обычных тем, что содержат не пустой
# каркас, а готовую формулировку: при миграции ошибка агента предсказуема —
# он «заодно улучшает» старый код и правит то, что трогать не просили.
# Формулировка, которая это останавливает, одна и та же в любом проекте.
migration_body() {
  cat <<'EOF'
# Переходный период: старое и новое сосуществуют

В проекте два контура. Правила для них разные, и это не временный
беспорядок, а осознанное состояние на время миграции.

## Где проходит граница

ЗАПОЛНИ: `app/Legacy/**` — старый контур, `app/Domain/**` — новый.
Без явной границы правило нерабочее: агент не угадает, что считать старым.

## Новый код

Пишется по общим правилам целиком: спецификация, тесты, гейты.
Скидок на «это же миграция» нет — иначе новый контур станет вторым старым.

## Старый код

- **Трогается только по прямой задаче.** Заодно, попутно, «раз уж открыл» —
  нет. Правка старого файла ради стиля создаёт диффы, которые никто
  не проверяет, и прячет в них настоящие изменения.
- **Перед изменением поведения — характеризационные тесты**: сначала
  фиксируем, как работает сейчас (включая странности), потом меняем.
  Процедура: скажи агенту «зафиксируй поведение этого модуля».
- **Стиль старого кода сохраняется.** Приводить его к новым конвенциям
  можно только вместе с переносом в новый контур.

## Перенос из старого в новое

Порядок обязателен, каждый шаг отдельным коммитом:

1. Характеризационные тесты на текущее поведение — зелёные на старом коде.
2. Новая реализация. Те же тесты — зелёные и на ней.
3. Переключение вызывающих мест.
4. Удаление старого. Процедура `safe-removal`: удалять то, на что
   не осталось ссылок, а не то, что «выглядит неиспользуемым».

Шаг 1 не пропускается никогда. Без него «поведение не изменилось» —
это мнение, а не факт.

## Что считается законченным переносом

ЗАПОЛНИ: критерий, по которому видно, что участок мигрировал. Например:
в `app/Legacy/Billing` не осталось файлов, все вызовы идут в `app/Domain`.
EOF
}

boundary_body() {
  cat <<'EOF'
# Зона заморозки: этот код не меняем

## Что заморожено

ЗАПОЛНИ: перечисли пути. Без списка правило не работает.

## Почему

ЗАПОЛНИ. Причина обязательна: она определяет, когда заморозку снимут.
Годные причины: нет тестов и нет способа их написать без стенда; код
переписывается в соседней ветке; модуль выводится из эксплуатации в срок.

Негодная причина: «страшно трогать». Это описание чувства, а не границы —
такая заморозка не снимется никогда.

## Что можно

- Читать и опираться при работе с другими участками.
- Исправлять аварии на проде — с явной задачей и обязательным тестом
  на воспроизведение сбоя.

## Что нельзя

- Рефакторинг, переименования, форматирование, обновление стиля.
- Правки «заодно» при работе с соседним кодом.
- Автоматические починки линтером по всему каталогу.

## Когда снимается

ЗАПОЛНИ: событие или дата. Заморозка без срока превращается в вечную —
и тогда через год никто не помнит, почему сюда нельзя.
EOF
}
...
new_rule() {
...

  {
    ...
    echo "---"
    echo
    # Шаблоны переходного периода дают готовую формулировку: ошибка агента
    # при миграции одна и та же везде, и текст, который её останавливает,
    # не зависит от проекта
    case "$kind" in
      migration)       migration_body ;;
      boundary|freeze) boundary_body ;;
      *)
...
EOF
  ...
  echo "  Вернуть обратно: /std-core:setup"
}
