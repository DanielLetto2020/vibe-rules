...
is_text_command() { # hd, sw, P
  ...
  if [[ "$hd" == git ]]; then
    git_sub "$P"
    case "$GSUB" in
      log|grep|show|diff|status|blame|shortlog|describe|rev-parse|ls-files|remote|branch|tag) return 0 ;;
    esac
  fi
  return 1
}
...
rule_git() { # <индекс>
  local j=$1 nT=${#T[@]} i k t v sub force=0 plus=0
  i=$((j + 1))
...
sql_check() { # <текст> <рядом клиент БД 0|1>
  ...
  ((${2:-0})) && [[ "$s" =~ $RE_SQL_CLIENT ]] && deny "$MSG_DB"
  return 0
}
...
analyze() { # <командная строка> <глубина>
  ...
  for ((k = 0; k < nseg; k++)); do
    ...
    PHD[k]=""; PTX[k]=""; PPT[k]=""
    if ((n == 0)); then
      # Только перенаправление: `> .claude/std-hooks-off`, `: > файл`
      hd=""; P=0; layer_rules
      continue
    fi
    strip_prefix
