...
ask_loss() { emit ask "$1"; }
ask()      { ((HOOKS_OFF)) && return 0; emit ask "$1"; }
...
rm_zone() { # <аргумент> -> system|outside|inside
  local p="$1" home="${HOME:-/root}" proj="${CLAUDE_PROJECT_DIR:-$PWD}" d

  # ~ и $HOME раскрываются здесь: оболочка раскроет их при запуске, и для
  ...
  # Хвостовой слэш каталог не меняет: /var/lib/ и /var/lib — одно и то же
  while [[ ${#p} -gt 1 && "$p" == */ ]]; do p="${p%/}"; done

  [[ "$p" == "/" || "$p" == "/*" ]] && { printf system; return; }
  [[ "$p" == "$home" || "$p" == "$home/*" || "$p" == "$home/.*" ]] && { printf system; return; }

  # Проект проверяется раньше системных каталогов: он сам может лежать в /opt
  # или /srv, и его build/ там ни при чём.
  [[ -n "$proj" && ( "$p" == "$proj" || "$p" == "$proj"/* ) ]] && { printf inside; return; }
  ...
  [[ "$p" == "$home"/* ]] && { printf outside; return; }

  for d in /bin /boot /dev /etc /home /lib /lib32 /lib64 /opt /proc /root /run /sbin /srv /sys /usr /var; do
    [[ "$p" == "$d" || "$p" == "$d"/* ]] && { printf system; return; }
  done

  case "$p" in
    /*|..|../*) printf outside; return ;;
  esac
  printf inside
}
...
apply_rules() { # <сегмент>
  ...
  if printf '%s' "$s" | grep -Eq 'git[[:space:]]+reset[[:space:]]+--hard'; then
    ask_loss "git reset --hard уничтожит незакоммиченные изменения. Подтверди."
  fi
  if printf '%s' "$s" | grep -Eq 'git[[:space:]]+clean[[:space:]]+[^;&|]*-[a-zA-Z]*[dx]'; then
    ask_loss "git clean удалит неотслеживаемые файлы — в том числе те, что ещё не добавлены в индекс и нигде не сохранены. Подтверди."
  fi
  ...
  if printf '%s' "$s" | grep -Eq '(^|[[:space:]])rm([[:space:]]|$)'; then
    ...
    if printf '%s' "$s" | grep -Eq '(^|[[:space:]])(-[a-zA-Z]*[rR][a-zA-Z]*|--recursive)([[:space:]]|$)'; then
      ...
      # Правило выше сравнивает строку с корнями и потому видит только их:
      # `rm -rf /var` блокировался, `rm -rf /var/lib/postgresql` проходил —
      # хотя для потерянных данных разницы никакой. Здесь цели разбираются
      # поимённо: каждая относится к своей зоне и получает своё решение.
      local tok zone
      for tok in $s; do
        ...
        case "$zone" in
          system)
            deny "Рекурсивное удаление системного пути: $tok. Это не откатывается ничем, кроме восстановления машины. Если каталог действительно надо снести — сделай это сам, вне сессии." ;;
          outside)
            ask_loss "Рекурсивное удаление за пределами проекта: $tok. Что там лежит, отсюда не видно, и под git оно, скорее всего, не находится — вернуть будет нечем. Подтверди путь." ;;
        esac
      done
    fi
  fi
  ...
  if printf '%s' "$s" | grep -Eq '(helm[[:space:]]+(uninstall|delete)|terraform[[:space:]]+destroy)'; then
    ask_loss "Команда сносит развёрнутое окружение. Подтверди, что это не продуктивный контур."
  fi
...
analyze() { # <командная строка> <глубина>
  ...
  if [[ ${#segs[@]} -eq 0 && -n "${line//[[:space:]]/}" ]]; then
    # Правила по неразобранной строке целиком — грубая проверка, но при
    # выключенных замках она единственная: вопрос там не задаётся, и без неё
    # неразобранное проходило бы вовсе без запретов.
    apply_rules "$line"
