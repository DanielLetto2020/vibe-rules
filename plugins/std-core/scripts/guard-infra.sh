...
# Исключения и дополнения словаря секретов. Строка в secret-allow выключает
# проверку коммита для всего, что её содержит, — это единственный слой с
# запретом, а не вопросом, и снимать его модель по ходу задачи не должна.
if printf '%s' "$REL" | grep -qE '(^|/)\.claude/secret-(allow|patterns)$'; then
  ask "Правка $REL меняет, что считается секретом. Строка в secret-allow снимает запрет на коммит для всего, что её содержит. Это решение человека: он видит, что именно разрешается и почему."
fi
# Состояние храповиков и отметка прохода гейтов. Планка опускается правкой
# одного числа, а отметка разрешает коммит без прогона: и то и другое должно
# быть видно человеку, а не происходить по ходу задачи.
if printf '%s' "$REL" | grep -qE '(^|/)\.claude/(\.ratchet\.json|\.debt\.json|\.gauntlet-pass)$'; then
  ask "Правка $REL меняет планку храповика или отметку прохода гейтов. Планку опускает человек, осознанно; модель её не трогает — даже если гейт красный и так было бы быстрее."
fi
...
if printf '%s' "$REL" | grep -qE '(^|/)(hooks\.json)$|(^|/)scripts/((guard|secret|session|rules)-[a-z-]+|precommit-secrets|scan-tree|ratchet|debt|gauntlet|diff-coverage)\.(sh|py)$'; then
