...
default_gates() {
  ...
  # он честно скажет «не проверено» (код 2), а прогон от этого не покраснеет.
...
for g in "${GATES[@]}"; do
  ...
  # Код 2 у гейта покрытия означает «мерить нечем»: отчёта нет, тесты гоняли
  # без него. Это не провал работы, но и не успех — прогон продолжается,
  # а в итоге стоит «не проверено». Молчаливое «пройден» здесь было бы
  # худшим вариантом: гейт есть, зелёный, а покрытие никто не считал.
  if [[ "$name" == "diff-coverage" && $rc -eq 2 ]]; then
    ylw "  не проверено: нет отчёта о покрытии"
    sed -n '1,6p' "$LOGDIR/$name.log" | sed 's/^/    /'
