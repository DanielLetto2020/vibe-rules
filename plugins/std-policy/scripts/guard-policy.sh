...
# Только целое число. Значение идёт в арифметику оболочки, а она раскрывает
# $(...) внутри индекса массива: строка "SIZE_KB[$(команда)]" из policy.json
# исполнялась. Политика — файл в репозитории, и правит его кто угодно,
# включая модель; хук не исполняет свой вход ни при каких значениях.
[[ "$MAXKB" =~ ^[0-9]+$ ]] || MAXKB=""
