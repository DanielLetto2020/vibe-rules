...
hdr "1/5 Права на исполнение скриптов"
missing=0
...
hdr "2/5 Структура модулей и frontmatter"
python3 "$ROOT/tests/lint-modules.py" || RC=1

hdr "3/5 Замки (unit-тесты хуков)"
bash "$ROOT/tests/test-hooks.sh" || RC=1

hdr "4/5 Автодетект стека и целостность связей"
bash "$ROOT/tests/test-link.sh" || RC=1

hdr "5/5 Валидация манифестов средствами Claude Code"
