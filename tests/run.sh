...
hdr "1/8 Страж приватных данных"
python3 "$ROOT/tests/no-private-data.py" || RC=1

hdr "2/8 Права на исполнение скриптов"
missing=0
...
hdr "3/8 Структура модулей и frontmatter"
python3 "$ROOT/tests/lint-modules.py" || RC=1

hdr "4/8 Замки (unit-тесты хуков)"
bash "$ROOT/tests/test-hooks.sh" || RC=1

hdr "5/8 Автодетект стека и целостность связей"
bash "$ROOT/tests/test-link.sh" || RC=1

hdr "6/8 Профили, храповик, единая настройка"
bash "$ROOT/tests/test-profile.sh" || RC=1

hdr "7/8 Политика стека"
bash "$ROOT/tests/test-policy.sh" || RC=1

hdr "8/8 Валидация манифестов средствами Claude Code"
