...
grep -q 'Что изменилось' <<<"$BODY" && ok "описание на русском" \
                                    || bad "язык описания" "нет русского заголовка"
# Второй язык в описании больше не поддерживается: документация одноязычная,
# и англоязычный хвост в релизе означал бы, что где-то остался текст, который
# никто не обновляет.
grep -qi 'English\|What changed' <<<"$BODY" \
  && bad "язык описания" "остался английский текст" \
  || ok "английского текста в описании нет"
grep -q 'START.md' <<<"$BODY" \
  && ok "ссылка на стартовый документ на месте" \
  || bad "ссылки" "нет ссылки на START.md"
